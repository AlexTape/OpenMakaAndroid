...
//! Read cv::Mat from binary
    ...
    cv::Mat transPointVecToMatHom(
            std::vector<cv::Point2f> &pt_vec); // Convert Point2f structure to Mat type in the homogeneous coordinates
...
//int checkPointsDistance(std::vector<cv::Point2f> &src_pts, std::vector<cv::Point2f> &dest_pts, double dist_threshold, std::vector<unsigned char>& status);	// The distance between two points after the affine transformation is not less than the threshold value a status to 0
    bool checkRectShape(
            std::vector<cv::Point2f> &rect_pt);    // Check a rectangular shape that projective transformation
    cv::Mat createMask(cv::Size img_size, std::vector<cv::Point2f> &pts);

    // Creating a mask surrounding the four points specified in the pts
    int checkInsideArea(std::vector<cv::Point2f> &points,
                        std::vector<cv::Point2f> &corner_pts,
                        std::vector<unsigned char> &status);

    bool checkPtInsideImage(cv::Size img_size,
                            std::vector<cv::Point2f> &pts);// judgment pts is there within the image area
//double erf(double x);	// error function

    void resizeMatChannel(cv::Mat &src_mat, cv::Mat &dest_mat,
                          double val = 0);    // Convert the number of channels to channel of src_mat. Add the val in the empty channel
    template<typename _Tp>
    void resizeMatChannelType(cv::Mat &src_mat,
                              cv::Mat &dest_mat,
                              double val = 0);    // Convert the number of channels to channel of src_mat. Add the val in the empty channel
    ...
    template<typename _Tp>
    void setChannelValueType(cv::Mat &dest_mat,
    ...
    template<typename _Tp>
    void decomposeHomographyType(cv::Mat &H_mat,
                                 cv::Mat &camera_matrix, cv::Mat &rotation, cv::Mat &translation);

    void decomposeHomography(cv::Mat &H_mat, cv::Mat &camera_matrix,
                             cv::Mat &rotation, cv::Mat &translation, cv::Point2f marker_center);
