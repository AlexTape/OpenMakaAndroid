...
int Controller::MAX_IMAGE_SIZE;
int Controller::IMAGE_SCALE;
...
// NEEDED
void Controller::initialize(cv::Mat &frame, std::string configPath) {
    ...
    cv::FileNode queryNode = storage["query"];
    Controller::MAX_IMAGE_SIZE = (int) queryNode["frameMaxSize"];
    Controller::IMAGE_SCALE = (int) queryNode["frameScale"];
    ...
    if (Controller::MODE_DEBUG) {
        ...
        cout << "::MAX_IMAGE_SIZE=" << Controller::MAX_IMAGE_SIZE << endl;
        cout << "::IMAGE_SCALE=" << Controller::IMAGE_SCALE << endl;
...
int Controller::displayFunction(cv::Mat &mRgbaFrame, cv::Mat &mGrayFrame) {
    ...
    if (MODE_OBJECT_DETECTION) {
        ...
        if (!MODE_TRACKING) {
            ...
            sceneFrame = new SceneFrame(mRgbaFrame, mGrayFrame, IMAGE_SCALE);
        ...
        if (MODE_TRACKING) {
            ...
            if (!STATE_TRACKING_OBJECT) {
                ...
                if (STATE_OBJECT_FOUND) {
                    ...
                    if (isInImage) {
                        ...
                        Drawer::drawContour(mRgbaFrame, sceneFrame->objectPosition, cv::Scalar(255));
            ...
            if (STATE_TRACKING_OBJECT) {
                ...
                Drawer::drawContour(mRgbaFrame, tracker->objectPosition, cv::Scalar(255));
    ...
    if (Controller::MODE_DEBUG) {
        cout << "DisplayFunction Time = " << clock->getMillis() << endl;
    }
